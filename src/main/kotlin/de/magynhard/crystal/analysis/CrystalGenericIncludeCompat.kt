...
internal object CrystalGenericIncludeCompat {
    ...
    fun includeEdgeCompatible(argType: String, paramType: String, context: PsiElement): Boolean? {
        ...
        val argArgs = splitGenericArgs(argType) ?: return null
        val paramArgs = splitGenericArgs(paramType) ?: return null
        if (argArgs.isEmpty() || paramArgs.isEmpty()) return null
    ...
    private fun typeParameterNames(declaration: PsiElement): List<String> {
        ...
        return parameters.typeReferenceList.map { it.text }
    }
