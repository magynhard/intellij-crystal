...
- [ ] **Trace the shard argument-count tail (3 findings, all in `lib/ameba`)** — kemal's own sources (src/ + spec/, including static_file_handler_spec.etag_with_coding:135) are finding-free. The remaining findings are distinct small causes, each needing its own trace: the `as_node` pool (variable_spec:102, "Missing 'source'"), the `as:` named argument (typos.cr:22), and `arg` in excessive_allocations_spec:10 ("expected at most 0, got 1"). (Fixed along the way: the tight bracket after a dot-call method name always binds as the receiver's index postfix — `identity.headers["Etag"], "gzip"` in a parenthesized argument list is TWO arguments of the enclosing call, so the array-literal-comma alternative now requires whitespace between the method name and the bracket; `Reference.new(node, scope)` resolves the sibling `Ameba::AST::Reference` through the program closure; macro-call arguments are no longer checked as runtime calls; proc-literal parameters resolve as local declarations.)
