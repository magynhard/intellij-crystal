...
- [ ] **Trace the shard argument-count tail (2 findings, both ameba spec fixtures)** — kemal's own sources (src/ + spec/, including static_file_handler_spec.etag_with_coding:135) and ameba's typos.cr `as:` DSL finding are clear. The remaining findings are distinct small causes, each needing its own trace: the `as_node` pool (variable_spec:102, "Missing 'source'") and `arg` in excessive_allocations_spec:10 ("expected at most 0, got 1"). (Fixed along the way: block bodies of macro invocations are macro data — ameba's `properties do bin_path nil, as: String? end` reads the named argument `as` as the property type through prop.named_args, so macro block bodies are diagnosed only via CrystalMacroContext.isInsideMacroCallBlock; earlier: the tight bracket after a dot-call method name always binds as the receiver's index postfix, `Reference.new(node, scope)` resolves the sibling `Ameba::AST::Reference` through the program closure, macro-call arguments are no longer checked as runtime calls, and proc-literal parameters resolve as local declarations.)
