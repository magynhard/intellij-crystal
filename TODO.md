...
- [ ] **Trace the amba-spec argument-count tail (10 findings)** — the remaining `lib/ameba` findings are distinct small causes, each needing its own trace: `Reference.new(node, scope)` (variable.cr:64) resolving to the stdlib `Reference` base class instead of the nested `Ameba::AST::Variable::Reference` (lexical-identity preference for nested types); `parse_args`/`as_node`/`etag_with_coding` calls (cmd_spec 238/252/264, variable_spec:102, static_file_handler_spec:135) whose pools miss the intended overloads; the `as:` named argument (typos.cr:22). (The admonition range-element findings were fixed — the dot-call bare-args guard now blocks the binary range operators.)
