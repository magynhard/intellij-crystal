...
- [ ] **Model ECR completion load context** — injected Crystal PSI is nonphysical and currently receives an empty effective-source snapshot, so DOT completion is conservatively suppressed instead of using an all-project fallback. Derive the compiling Crystal caller's forward require closure, or define another exact host load-context model, before restoring type-object and value DOT candidates inside ECR tags.
