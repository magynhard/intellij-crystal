...
- [ ] **Trace the amba-spec argument-count tail (6 findings)** — the remaining `lib/ameba` findings are distinct small causes, each needing its own trace: `Reference.new(node, scope)` (variable.cr:64) resolving to the stdlib `Reference` base class instead of the nested `Ameba::AST::Variable::Reference` (lexical-identity preference for nested types); `as_node`/`etag_with_coding` calls (variable_spec:102, static_file_handler_spec:135) whose pools miss the intended overloads; the `as:` named argument (typos.cr:22). (The admonition range-element findings, the annotated-source array-comma findings, and the cmd_spec loose-array findings were fixed — the dot-call bare-args guard blocks the binary range operators, the argument extraction includes the leading array literal, and the loose-array bare-argument alternative accepts whitespace-separated arrays.)
