...
- [ ] **Trace chained `env.status(...).json(...)` argument binding (8 findings, down from a merged cluster)** — the headless kemal audit reports "Missing required argument(s): 'status_code'" on the named-tuple keys of `halt env.status(500).json({error: "..."})` and `env.status(:not_found).json({...})` (helpers_spec 52/65/78/93/126, response_helpers_spec 172/197/209). The required `status_code` parameter can only come from `HTTP::Server::Response#status_code=(status_code : Int32)` — a setter no `json` call should resolve to. Capture the exact DOT-call resolution (receiver type, overload pool, and which pool member demands `status_code`) on the full require closure; distinguish a chain-receiver mis-type from wrong-overload selection before changing resolver fallback semantics.
- [ ] **Investigate operator-call arity on inferred stdlib values (1 finding)** — the headless kemal audit reports "Too many arguments: expected at most 0, got 1" for `(Time.monotonic - start).total_milliseconds` in `spec/run_spec.cr`. Reproduce the exact highlighted PSI and determine why `Time::Span#-(Time::Span)` is replaced by an implicit zero-argument constructor before changing resolver fallback semantics.
- [ ] **Investigate `path_handler_spec:157` union-typed comparison (1 finding)** — "Type mismatch: expected 'Int32' or 'HTTP::Handler' or ..." on a multi-union comparison; map the highlighted element to the exact overload slot before deciding whether the union handling or the overload selection is at fault.
