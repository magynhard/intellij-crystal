...
- [ ] **Repair remaining lib-body member families** — after the `constant_assignment` fix the
  ...
  (1) untyped positional fun parameters (`fun strerror_r(Int, Char*, SizeT) : Int`),
  (2) `fun name = symbol` external aliases (`fun iconv = libiconv(...)`,
  `fun realpath = "realpath$DARWIN_EXTSN"(...)` — including string-literal symbols),
  (3) uppercase Windows/LLVM function names (`fun GetConsoleScreenBufferInfo(...)`,
  `fun LinkInMCJIT`), (4) keyword-named functions (`fun select(...)` — the identifier
  alternative must tolerate word-keyword spellings in fun position only),
  (5) `@[Flags]`-style annotations inside `lib` bodies,
  (6) `{% %}` / `{{ }}` macro-control/interpolation in `lib` bodies.
