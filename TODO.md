...
- [ ] **Trace the shard argument-count tail (4 findings, all in `lib/`)** — kemal's own sources (src/ + spec/) are finding-free. The remaining findings are distinct small causes, each needing its own trace: the `as_node` pool (ameba variable_spec:102, "Missing 'source'"), `etag_with_coding` (kemal static_file_handler_spec:135, "Missing 'coding'"), the `as:` named argument (ameba typos.cr:22), and `Memory.usage`/`arg` in excessive_allocations_spec:10 ("expected at most 0, got 1"). (Fixed along the way: `Reference.new(node, scope)` now resolves the sibling `Ameba::AST::Reference` through the program closure; macro-call arguments (`property … logging …`) are no longer checked as runtime calls; proc-literal parameters (`error` in `handler.call(context, error)`) resolve as local declarations.)
