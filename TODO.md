...
- [ ] **Remaining indexed-audit parse errors** — the escape fix removed the escaped `-\{% / \{{`-statements in primitives.cr (lines 101/146); the indexed corpus keeps 180 raw errors across 127 files, mostly compiler-tree families (`struct {{num.id}}`-style declaration bodies, macro-controlled members). They need per-cause traces, each Sym family treated like the escape fix.
