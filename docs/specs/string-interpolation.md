...
| 22 | `"#{File.basename(`#{__DIR__}/x`)}"` | `INTERPOLATION` → `BACKTICK` → `INTERPOLATION` | ✅ Done — the backtick opens a command inside interpolation; the command's own `#{}` nests via the shared `BACKTICK`/`INTERPOLATION` states (`debug/driver.cr`) |
