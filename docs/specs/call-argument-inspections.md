...
- The split `call_args COMMA bare_argument_list` shape (`restrict (X), context`) keeps the post-comma tail outside `call_args`; arity and type checks must read it via `trailingBareArguments`, or a present argument is reported missing and a mistyped one goes unchecked.
...
- Receiver-less `new` in class context (`new [name], global` in ast.cr) resolving through the same combined pool; explicit-receiver calls and instance-method bodies keep their established paths.
...
- Simple record constructors validated through exact current-file `RecordFallback`, including argumentless calls and lexical collisions with indexed types. Field names may be keyword tokens (`record Span, start : Int32, end : Int32`) or instance/class-variable shorthands (`record Point, @x : Int32, @@y : Int32`): the shared `CrystalPsiUtils.recordFieldInfo` reads the name before the colon as an `IDENTIFIER`, any `keyword_identifier` token, or a sigil-stripped instance/class variable, so the `end`/`x`/`y` fields count towards arity and type checks instead of being dropped.
