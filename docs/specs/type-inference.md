...
## Variable Hover Type Info

### Goal

Show the inferred type of a variable when hovering over it in the editor,
using the same visual pattern as parameter hover info.

### Display Format

When hovering over a variable identifier, display a two-line popup:
...
- **Line 1:** The inferred type name, rendered as a clickable link (resolves to the
  class definition via `CrystalClassIndex`), followed by ` (Variable)` in muted gray.
- **Line 2:** The variable name as plain text.

This mirrors the existing parameter hover format:
```
String (Parameter)
age
```
...
1. In `getCustomDocumentationElement` — detect variable identifiers via
   `isVariableIdentifier()` check, return the element itself.
...
4. `buildVariableSignatureHtml` renders the same HTML pattern as parameter hover:
   clickable type link + `(Variable)` label + variable name.
