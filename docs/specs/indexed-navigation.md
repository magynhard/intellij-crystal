...
`CrystalLegacyStdlibExcludePolicy` protects the first project open while a legacy module library still exists. It activates only when a module-level library is named exactly `Crystal StdLib`, resolves the configured stdlib root, and excludes only the known non-user-facing distribution directories `compiler`, `crystal`, `lib_c`, `lib_z`, `ll`, `llvm`, `gc`, and `samples`. Without that exact legacy library it excludes nothing, so opening the Crystal compiler source as normal project content is unaffected.
