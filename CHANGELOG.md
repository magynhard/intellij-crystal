...
- **Keep each DOT completion on one source snapshot** — receiver inference, instance/static method collection, and synthetic constructor classification now share one type-resolution session. Nonphysical ECR fragments remain conservative: same-fragment records do not bypass the empty effective-source set to offer `new`.
