...
- **Prevent endless first-open stdlib indexing** — the filtered synthetic library is now the sole stdlib root source. A pre-load project converter removes legacy persisted `Crystal StdLib` module entries before Workspace Model can queue the broad `/usr/lib/crystal` root, while SDK/reindex actions refresh only filtered `.cr` roots through IntelliJ's additional-library API. Force Re-index is ignored outside Crystal projects and honors cancellation while collecting and scheduling files. The post-startup module-library migration and indefinite smart-mode waiter have been removed.
