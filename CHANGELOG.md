...
- **Navigate every exact DOT-call overload** — `CrystalDotCallReference` is now polyvariant for regular static, instance, and explicit constructor overloads. IntelliJ receives all exact targets for its chooser, while single-target resolution succeeds only when unique and constructor precedence remains unchanged.
