...
- **Analyze expression DOT receivers without changing completion results** — internal completion infrastructure now consumes every component of flattened postfix prefixes before a member-access dot, including constructors, long mixed chains, and qualified generic type objects; unsupported tails and ambiguous overloads remain unknown. It preserves complete reachable control-flow type sets and annotation unions, including grouped unions and internal names of externally named parameters, while normalizing generic values to their outer lookup type. Contributor dispatch and multi-type candidate lookup are not integrated yet.
