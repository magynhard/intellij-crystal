...
- **Cache require-aware effective source snapshots** — a project-level require DAG now combines a dependency-version-validated `prelude.cr` foundation with each file's forward-only transitive require closure. Immutable snapshots are reused across ordinary body edits, changed or deleted prelude dependencies rebuild before publication, deep cycles use stack-safe iterative traversal, unresolved or invalid files cannot leak stale sources, and changed require fingerprints invalidate only reverse dependents. Completion uses only an already cached stdlib root and never starts `crystal env` discovery.
