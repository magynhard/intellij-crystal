...
- **`require` keyword and path completion** — typing `req<caret>` at an independent file-scope statement now offers the `require` compiler keyword, including inside top-level compile-time macro conditions. The synthesized keyword is excluded from runtime control flow, blocks, type/callable/macro-definition bodies, DOT calls, namespace access, type annotations, assignment values, arguments, conditions, and unfinished multiline expressions; real methods named `require` remain available through normal DOT completion. Selecting the keyword inserts `require ""` and immediately opens path completion. A leading `.` or `/` lists relative `.cr` files and directories, while other prefixes list project shards and cached Crystal stdlib entries, with multi-segment directory traversal in both modes.
