...
- **Cache require-aware effective source snapshots** — a project-level require DAG now combines one reusable `prelude.cr` foundation with each file's forward-only transitive require closure. Immutable snapshots are reused across ordinary body edits, cycles are deduplicated, unresolved or invalid files cannot leak stale sources, and changed require fingerprints invalidate only reverse dependents.
