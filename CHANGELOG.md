...
- **Stdlib over-indexing on Crystal ≥ 1.20 hangs project open / blocks completion** — Crystal 1.20+ ships stdlib files directly under `/usr/lib/crystal` (no `src/` subdirectory) alongside the compiler source (`compiler/`), CLI tooling (`crystal/`), C ABI bindings (`lib_c/`, `lib_z/`), LLVM bindings (`llvm/`, `ll/`), and the GC interface (`gc/`). The plugin previously indexed the whole tree — about 1613 files instead of the actual ~460 stdlib files. With the new `CrystalTopLevelMethodIndex` triggering a one-time StubIndex rebuild on plugin update, this caused multi-minute "Analyzing project…" hangs that blocked the EDT when a user typed a free-text completion character. Fixed by introducing `CrystalStdlibRoots`, which enumerates only the user-facing stdlib roots (top-level `.cr` files + non-excluded subdirectories). Existing "Crystal StdLib" libraries from previous plugin versions are auto-migrated (removed + recreated with the filtered multi-root set) on the next project open. Additionally, free-text completion's stub-index query is now gated on `DumbService.isDumb(project)`, so typing a character while indexing is still in progress no longer triggers a synchronous StubIndex build — the user sees current-file top-level methods immediately, and cross-file/stdlib helpers appear once indexing finishes. Finally, the new index is warmed on a background pooled thread after project open (`warmTopLevelMethodIndex` calls `runReadActionInSmartMode { getAllKeys(...) }`) so the first free-text completion call doesn't trigger a synchronous build.
