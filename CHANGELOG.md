...
- **Design require-aware DOT completion** — define a cached per-file require graph with a reusable `prelude.cr` foundation so core `String`, numeric, and collection methods remain available while optional stdlib, shard, and project reopenings contribute methods only through the current file's forward require closure.
