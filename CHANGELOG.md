...
- **Analyze expression DOT receivers without changing completion results** — internal completion infrastructure now locates complete receiver PSI before a member-access dot, distinguishes exact type objects from inferred value type sets, expands only top-level unions, and normalizes generic values to their outer lookup type. Contributor dispatch and multi-type candidate lookup are not integrated yet.
