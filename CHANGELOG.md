...
- **Validate direct argumentless calls without receiver false positives** — unqualified calls such as `greet` now report signature-required arguments while local declarations, types, macros, same-line or multiline DOT receivers, and namespace roots remain excluded through scoped StubIndex lookups.
