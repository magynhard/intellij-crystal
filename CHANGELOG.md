...
- **Analyze expression DOT receivers without changing completion results** — internal completion infrastructure now follows complete flattened postfix prefixes before a member-access dot, including constructors, chained method results, and qualified generic type objects. It distinguishes exact type objects from inferred value type sets, preserves annotation unions in source order, and normalizes generic values to their outer lookup type. Contributor dispatch and multi-type candidate lookup are not integrated yet.
