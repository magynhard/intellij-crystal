...
- **Validate argumentless calls against method signatures** — unqualified calls such as `greet` and exact constant receiver calls such as `Factory.create` now report missing signature-required parameters. Explicit defaults remain optional, nilable annotations remain required without a default, splats accept zero arguments, matching overloads suppress diagnostics, and local declarations, types, macros, or inapplicable receiver methods continue to shadow or exclude indexed candidates according to the documented resolution rules.
