...
- **Prepare union receiver method candidates without changing contributor dispatch** — instance-method lookup now accepts ordered exact type roots, uses one cached neutral hierarchy session, and merges duplicate canonical signatures while retaining overloads and namespace isolation. Primitive and generic outer roots use the same indexed path. Expression DOT completion does not dispatch through this API yet.
- **Centralize scoped expression receiver analysis without changing completion results** — a neutral memoized type-set resolver now owns lexical variable flow, complete control-flow sets, exact receiver/implicit-self method returns, overload conservatism, and recursion safety using cached StubIndex lookups. Completion retains only receiver/postfix decomposition and type-object handling; legacy inspection and string inference APIs are compatibility adapters. Contributor dispatch through the receiver analysis remains unintegrated.
- **Share conservative DOT receiver normalization through neutral PSI infrastructure** — inspections now use `CrystalReceiverExpression` for transparent single-expression grouping and exact constant/generic roots while assignments, multi-expression groups, dynamic namespace roots, and ambiguous receivers remain opaque. This prepares shared receiver analysis for later expression DOT completion; completion integration is not included yet.
